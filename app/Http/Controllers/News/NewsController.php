...
namespace ITATS\PraktikumTeknikSipil\Http\Controllers\News;
...
class NewsController {
    ...
    public function Detail($category, $id, $slug) {
        ...
        foreach($news as $newItem) {
            if($category == $newItem['category'] && $id == $newItem['id'] && $slug == $newItem['slug']) {
                $item = $newItem;
                break;
            }
        }
        ...
        if($item != null) {
            View::render('homepage.detailNews', [
                'userRole' => $userRole,
                'title' => 'News Detail | Lab Praktikum Teknik Sipil',
                'notification' => $notification,
                'item' => $item
            ]);        
        } else {
